module MainModule
    ...
    function load_data()
        file_path = "C:\\...Your_Own_File_Path\\earthchem_download_32495.csv"
    ...
    function safe_categorical(v)
        ...
        @assert scitype(cat_v) <: AbstractVector{<:OrderedFactor} "Type Convertion Failure"
    ...
    function main()
        # 1. Load data
        df = load_data()
        @info "Data loading complete，sample number：$(nrow(df))"

        # 2. Split data set
        train, test = partition(eachindex(df.ROCK), 0.8, shuffle=true)
        train_df = df[train, :]
        test_df = df[test, :]
        @info "Data set split complete，training set：$(nrow(train_df))，test set：$(nrow(test_df))"
