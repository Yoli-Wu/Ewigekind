...
function safe_load_model(path)
    isfile(path) || error("Model document does not exist: $path")
    filesize(path) > 0 || error("Empty model document: $path")
    try
        return XGBoost.load(XGBoost.Booster, path)
    catch e
        @error "Model loading failure" exception=(e, catch_backtrace())
        rethrow(e)
    end
end
...
function validate_features(columns, expected)
    ...
    isempty(mismatch) || error("Features don't match，missing：$mismatch")
end
...
# Please fill in the actual data HERE!（sample data for demo）
sample_values = Dict(
...
function safe_predict(booster, dmat, levels)
    try
        ...
        @assert size(proba, 1) == 1 "Should only have ONE sample"
        @assert size(proba, 2) == length(levels) "Category numbers don't match"
        ...
        @error "Prediction Failure" exception=(e, catch_backtrace())
        rethrow(e)
    end
end
